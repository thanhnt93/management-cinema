...
public interface ITicketRepository extends JpaRepository<Ticket, Integer> {
    ...
    /**
     * Phương thức này dùng để update vé dựa vào username
     * creator: ThanhNT
     *
     * @param username: dùng để lấy username của người dùng khi thực hiện update
     */
    @Modifying
    @Query(value = "UPDATE ticket SET status_ticket = 1 " +
            ...
            "            WHERE ticket.is_delete = 0 AND ticket.status_ticket = 0) AS x)", nativeQuery = true)
    void updateTicketByUserName();

    /**
     * Phương thức này dùng để lấy thông tin ticket theo username
     * creator: ThanhNT
     *
     * @param username: dùng để lấy username của người dùng khi lấy vé
     * @return: trả về 1 kết quả của vé
     */
    @Query(value = "SELECT * " +
            "FROM ticket " +
            "JOIN customer ON ticket.customer_id = customer.id " +
    ...
    /**
     * Phương thức này dùng để lấy thông tin người dùng và vé xem phim theo username
     * creator: ThanhNT
     *
     * @param username: dùng để lấy username của người dùng khi lấy thông tin người dùng đặt vé
     * @return: trả về 1 list vé người dùng đặt chưa thanh toán
     */
